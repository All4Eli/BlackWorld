...
export async function POST(request) {
  ...
  try {
    const { data: updatedLair, error } = await LairsDal.upgradeLairTransactional(userId);
    if (error) {
      if (error.message === 'NO_LAIR') return NextResponse.json({ error: 'BAD_REQUEST', message: 'No lair to upgrade' }, { status: 400 });
    ...
    return NextResponse.json({ success: true, lair: updatedLair });
  } catch (err) {
    console.error(err);
